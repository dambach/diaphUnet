...
class U_Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder
        #self.up_conv1 = torch.nn.ConvTranspose2d(128*C, 64*C, kernel_size = kernel[-1,:], stride = 2).to(device)
        #x9 = self.up_conv1(x8) # 3x3
        #y1 = crop_image(x7, x9)
        #x10 = torch.cat((y1, x9), dim = 1)
        #x11 = self.conv9(x10)
        
        #self.up_conv2 = torch.nn.ConvTranspose2d(64*C, 32*C, kernel_size = kernel[-2,:], stride = 2).to(device)
