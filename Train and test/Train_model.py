...
Original file is located at
    https://colab.research.google.com/drive/1qiNaIdSgUXuGEVRHTQE-k_qS3oN3KQME
"""
...
for param in model.parameters():
  if param.ndim < 2:
    param.data = 0.5*torch.randn(param.data.size()).to(device).to(device)
  else:
    param.data = torch.nn.init.xavier_uniform_(param).to(device).to(device)
    #param.data = torch.nn.init.xavier_uniform_(param.data).to(device)

def train_model(model, xtrain, ytrain, xtest, ytest, num_epoch, batch, loss_fn, save_epoch, optimizer):
  train_losses = []
  test_losses = []
  acc_train = []
  acc_test = []
  ...
  for epoch in range(1, num_epoch + 1):
    ...
    for u, v, mu, mv in zip(xtrain, xtest, ytrain, ytest):
      ...
      for img1, mimg1 in zip(train_loader, mtrain_loader):
        img1 = Variable(img1).type(torch.FloatTensor).to(device)
        optimizer.zero_grad()
        y_pred1 = model(img1)
      ...
      for img2, mimg2 in zip(test_loader, mtest_loader):
        img2 = Variable(img2).type(torch.FloatTensor).to(device)
        y_pred2 = model(img2)
